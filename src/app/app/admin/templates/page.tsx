...
export default function AdminTemplatesPage() {
  ...
  function addField(key = "patientName") {
    const option = FIELDS.find(([value]) => value === key) ?? FIELDS[0];
    setFields((current) => [...current, {
      id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      key: option[0], label: option[1], x: 8, y: 8 + current.length * 8, width: 32, height: 5, fontSize: 10, align: "left",
    }]);
  }
  ...
  async function save() {
    ...
    const template: DocumentTemplate = {
      ...
      kind, label: label.trim(), layout: fileData ? "uploaded-pdf" : "navayu-letterhead", description: description.trim(),
      ...
      <div className="grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)_340px]">
        ...
        <Panel title="Visual placement">
          <div ref={previewRef} onPointerMove={moveDrag} onPointerUp={stopDrag} onPointerLeave={stopDrag} className="relative aspect-[0.707] overflow-hidden rounded-md border border-[var(--attio-border)] bg-white">
            {fileData && mimeType.includes("pdf") ? <iframe src={fileData} title="Template preview" className="pointer-events-none absolute inset-0 h-full w-full" /> : <div className="absolute inset-0 flex items-center justify-center p-6 text-center text-[12px] text-[var(--attio-text-secondary)]"><div><Upload className="mx-auto mb-2 size-6" /><p>Upload a PDF to see the live canvas.</p><p className="mt-1 text-[10px]">Then drag each field into its exact position.</p></div></div>}
            {fields.map((field) => <div key={field.id} onPointerDown={(event) => startDrag(event, field)} className="absolute cursor-move select-none border border-emerald-500 bg-emerald-100/80 px-1 py-0.5 text-[10px] text-emerald-900 shadow-sm" style={{ left: `${field.x}%`, top: `${field.y}%`, width: `${field.width}%`, minHeight: `${field.height}%`, fontSize: `${field.fontSize ?? 10}px`, textAlign: field.align ?? "left" }}><GripVertical className="mr-0.5 inline size-2.5" />{field.label}</div>)}
          </div>
          ...
          <div className="mt-3 max-h-[260px] space-y-2 overflow-y-auto">{fields.map((field) => <div key={field.id} className="rounded-md border border-[var(--attio-border)] p-2"><div className="flex items-center gap-2"><select value={field.key} onChange={(event) => { const option = FIELDS.find(([key]) => key === event.target.value); updateField(field.id, { key: event.target.value, label: option?.[1] ?? event.target.value }); }} className="h-7 min-w-0 flex-1 rounded border border-[var(--attio-border)] px-1 text-[11px]">{FIELDS.map(([key, fieldLabel]) => <option key={key} value={key}>{fieldLabel}</option>)}</select><button type="button" onClick={() => setFields((current) => current.filter((item) => item.id !== field.id))} className="text-red-600" aria-label={`Remove ${field.label}`}><Trash2 className="size-3.5" /></button></div><div className="mt-2 grid grid-cols-4 gap-1">{(["x", "y", "width", "height"] as const).map((key) => <label key={key} className="text-[9px] uppercase text-[var(--attio-text-secondary)]">{key}<input type="number" min={0} max={key === "x" || key === "y" ? 100 : undefined} value={field[key]} onChange={(event) => updateField(field.id, { [key]: Number(event.target.value) })} className="mt-0.5 h-7 w-full rounded border border-[var(--attio-border)] px-1 text-[10px]" /></label>)}</div></div>)}</div>
        </Panel>
      </div>
    </PageChrome>
  );
